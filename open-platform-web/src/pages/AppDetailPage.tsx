...
export function AppDetailPage() {
  ...
  const update = useMutation({ mutationFn: (input: AppUpdateInput) => platformApi.updateApp(appId, input, session!.csrf_token), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['app', appId] }); queryClient.invalidateQueries({ queryKey: ['apps'] }); setEditOpen(false); setAction(null) } })
  const createKey = useMutation({ mutationFn: (input: KeyCreateInput) => platformApi.createKey(appId, input, session!.csrf_token), onSuccess: (value) => { setCreateOpen(false); setSecret(value); queryClient.invalidateQueries({ queryKey: ['keys', appId] }) } })
  const rotate = useMutation({ mutationFn: (id: string) => platformApi.rotateKey(id, session!.csrf_token), onSuccess: (value) => { setAction(null); setSecret(value); queryClient.invalidateQueries({ queryKey: ['keys', appId] }) } })
  const revoke = useMutation({ mutationFn: (id: string) => platformApi.revokeKey(id, session!.csrf_token), onSuccess: () => { setAction(null); queryClient.invalidateQueries({ queryKey: ['keys', appId] }) } })
  ...
  return (
    <div className="console-page">
      ...
      <EditAppDialog app={record} open={editOpen} onOpenChange={setEditOpen} pending={update.isPending} error={update.error?.message} onSubmit={(input) => update.mutate(input)} />
...
function EditAppDialog({ app, open, onOpenChange, pending, error, onSubmit }: { app: PlatformApp; open: boolean; onOpenChange: (open: boolean) => void; pending: boolean; error?: string; onSubmit: (input: AppUpdateInput) => void }) {
  function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const data = new FormData(event.currentTarget); onSubmit({ display_name: String(data.get('display_name')), owner_itcode: String(data.get('owner_itcode')), description: String(data.get('description') || '') }) }
  return <Dialog open={open} onOpenChange={onOpenChange} title="编辑应用" description={`应用标识 ${app.app_name} 不可修改。`}><form className="form-grid" onSubmit={submit}><label>显示名称<input name="display_name" required defaultValue={app.display_name} /></label><label>负责人 itcode<input name="owner_itcode" required defaultValue={app.owner_itcode} /></label><label className="full-field">描述<textarea name="description" rows={4} defaultValue={app.description ?? ''} /></label>{error && <div className="form-error full-field">{error}</div>}<div className="dialog-actions full-field"><button className="button button-secondary" type="button" onClick={() => onOpenChange(false)}>取消</button><button className="button button-primary" type="submit" disabled={pending}>{pending ? '正在保存' : '保存更改'}</button></div></form></Dialog>
}
