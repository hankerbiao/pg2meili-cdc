...
export function AppDetailPage() {
  ...
  return (
    <div className="console-page">
      ...
      <ConfirmDialog open={action?.type === 'delete'} onOpenChange={(open) => !open && setAction(null)} title="删除应用" description={`删除 ${record.display_name} 将撤销全部 API Key，删除其 Meilisearch 索引并回收独立数据库 schema，此操作不可恢复。`} confirmLabel="确认删除" danger pending={remove.isPending} onConfirm={() => remove.mutate()} />
