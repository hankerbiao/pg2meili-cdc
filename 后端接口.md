...
```bash
curl -X POST 'http://localhost:8091/api/v1/testcases' \
...
```bash
curl -X PUT 'http://localhost:8091/api/v1/testcases/case_1001' \
-H 'Content-Type: application/json' \
-H 'Authorization: Bearer YOUR_ACCESS_TOKEN' \
-d '{
    "id": "case_1001",
...
```bash
curl -X DELETE 'http://localhost:8091/api/v1/testcases/case_1001' \
-H 'Authorization: Bearer YOUR_ACCESS_TOKEN'
```
